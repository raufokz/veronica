    ...
    <section className="bg-ink text-white">
      <div className="container-app py-20 md:py-28">
        <div className="max-w-2xl">
          ...
          {step === "done" ? (
            <p className="mt-8 rounded-lg border border-white/15 bg-white/5 p-6 font-display text-xl">
            ...
            <form onSubmit={handleAddressSubmit} className="mt-8 flex flex-col sm:flex-row gap-3">
              ...
              <button
                type="submit"
                className={cn(buttonVariants({ size: "lg" }), "rounded-full bg-brand hover:bg-brand/90 text-white h-12 px-6")}
              >
            ...
            <form onSubmit={handleSubmit(onDetailsSubmit)} className="mt-8 space-y-4 max-w-md" noValidate>
              ...
              <button
                ...
                className={cn(buttonVariants({ size: "lg" }), "rounded-full bg-brand hover:bg-brand/90 text-white h-11 px-6 w-full sm:w-auto")}
              >
                {isSubmitting ? "…" : t(dict.homeValue.cta, lang)}
              </button>
            </form>
          )}
        </div>
